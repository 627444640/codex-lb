...
def test_session_store_rejects_legacy_cookie_without_credential_fingerprint(monkeypatch) -> None:
    monkeypatch.setattr(dashboard_auth_service_module, "time", lambda: 1_700_000_000)
    legacy = TokenEncryptor().encrypt(json.dumps({"exp": 1_700_000_100, "pw": True, "tv": True})).decode("ascii")
    assert DashboardSessionStore().get(legacy) is None
