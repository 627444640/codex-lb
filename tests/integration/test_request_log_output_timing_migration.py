...
PARENT = "20260914_000000_add_request_log_pricing_metadata"
FIELDS = {"latency_first_output_ms", "output_delta_count"}
            ...
            row = (
                await connection.execute(
                    text(
                        ...
                        "cost_usd, latency_first_output_ms, output_delta_count FROM request_logs"
                    )
                )
            ).one()
            assert tuple(row) == (120, 110, 2005, 2000, 0.123456789, None, None)
